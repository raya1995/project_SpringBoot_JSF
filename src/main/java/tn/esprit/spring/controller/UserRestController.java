...
import tn.esprit.spring.services.FollowServiceImpl;
import tn.esprit.spring.services.NotificationServeur;
import tn.esprit.spring.services.RaitingUserServiceImpl;
...
@Join(path = "/", to = "/gererAd/AddHouseRent.jsf")
@RestController
public class UserRestController {
			...
			if (user instanceof Admin ) {
				...
				if (((Client) user).isBlock()) {
					if(((Client) user).getDescriptionBlock().equals("security problem")) {
						...
						navigateTo = "/client/block.xhtml?faces-redirect=true";
						loggedIn = true;
						this.user = user;

						this.client = ((Client) user);
					}if(((Client) user).getDescriptionBlock().equals("you have 3 reclamation try to connect with th adminstation")) {
						FacesMessage facesMessage = new FacesMessage(
								"you have 3 reclamation try to connect with th adminstation.");
						FacesContext.getCurrentInstance().addMessage("form:btn", facesMessage);
					}
					

				} else {
					navigateTo = "/client/basic.xhtml?faces-redirect=true";
					connectedUserService.newUserConnected();
				...
				navigateTo = "/pages/agent/welcome.xhtml?faces-redirect=true";
				loggedIn = true;
	...
	public String afficherProfile(Long id) { 
		...
		if(inviteClient.equals(ClientConnecte)) {
			navigateTo = "/client/welcome.xhtml?faces-redirect=true";
		}else {
			navigateTo = "/client/profileUser.xhtml?faces-redirect=true";
		}
	...
	public String afficherProfile1(Long id) { 
		String navigateTo = "null";
		inviteClient=null;
		idInvite=id;
		System.out.println(id);
